...
export default function App() {
  ...
  // ── 共通スタイル ──────────────────────────────────
  const inputStyle = {
    width:"100%", background:"#101D33", border:"1px solid #2C3E5F",
        ...
        <div style={{display:"flex", gap:6, marginBottom:7, alignItems:"center"}}>
          {[["action","営業アクション"],["product","商品"],["contact","連絡手段"]].map(([t,l])=>(
            <button key={t} onClick={()=>setCatTab(t)} style={{
              ...
              background:catTab===t?"#2C3E5F":"transparent", color:catTab===t?"#E8EAF0":"#5D6D8C",
            }}>{l}</button>
          ))}
          <button onClick={()=>setCatFilter("all")} style={{
            marginLeft:"auto", padding:"4px 11px", borderRadius:6, border:"none", cursor:"pointer", fontSize:11, fontWeight:700,
            background:catFilter==="all"?"#FFD700":"#1C2C48", color:catFilter==="all"?"#0A1220":"#8494B0",
          }}>すべて</button>
        </div>
          ...
          {Object.entries(catTab==="action"?ACTION_CATEGORIES:catTab==="product"?PRODUCT_CATEGORIES:CONTACT_CATEGORIES).map(([key,cat])=>{
            ...
            return (
              <button key={key} onClick={()=>setCatFilter(key)} style={{
                ...
                background:active?cc+"18":"#16263F", color:active?cc:"#8494B0",
  ...
  function CategoryPicker() {
    ...
    return (
      <div>
        <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:4}}>カテゴリ <span style={{color:"#5D6D8C", fontWeight:400}}>(複数選択・選択なしもOK)</span></label>
        <div style={{fontSize:10, color:"#5D6D8C", marginBottom:5}}>営業アクション</div>
          ...
          {Object.entries(ACTION_CATEGORIES).map(([k,v])=>{
            ...
            return (
              <button key={k} onClick={()=>toggleCat(k)} style={{
                ...
                background:active?"#2C3E5F":"#101D33",
                color:active?"#E8EAF0":"#5D6D8C",
                outline:active?"1px solid #4FC3F7":"none",
              }}>{v.icon} {v.label}</button>
            );
          })}
        </div>
        <div style={{fontSize:10, color:"#5D6D8C", marginBottom:5}}>商品</div>
        <div style={{display:"flex", gap:6, flexWrap:"wrap", marginBottom:10}}>
          {Object.entries(PRODUCT_CATEGORIES).map(([k,v])=>{
            ...
            return (
              <button key={k} onClick={()=>toggleCat(k)} style={{
                padding:"5px 10px", borderRadius:16, border:"none", cursor:"pointer", fontSize:11, fontWeight:600,
                background:active?v.color+"18":"#101D33", color:active?v.color:"#5D6D8C",
                outline:active?`1px solid ${v.color}`:"none",
              }}>{v.icon} {v.label}</button>
            );
          })}
        </div>
        <div style={{fontSize:10, color:"#5D6D8C", marginBottom:5}}>連絡手段</div>
  ...
  // ════════════════════════════════════════════════════
  if (loadError) return (
    <div style={{minHeight:"100vh", background:"#0A1220", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:14, padding:20, textAlign:"center"}}>
      <div style={{fontSize:34}}>⚠️</div>
      <div style={{color:"#FF5252", fontSize:16, fontWeight:800}}>データベースに接続できません</div>
      <div style={{color:"#E8EAF0", fontSize:13, background:"#16263F", border:"1px solid #2C3E5F", borderRadius:10, padding:"10px 16px", maxWidth:480, wordBreak:"break-all"}}>
      ...
      <button onClick={()=>window.location.reload()} style={{marginTop:6, padding:"9px 22px", borderRadius:10, border:"none", cursor:"pointer", fontSize:13, fontWeight:700, background:"linear-gradient(135deg,#FFD700,#FF8C00)", color:"#0A1220"}}>再読み込み</button>
    </div>
  );

  if (loading) return (
    <div style={{minHeight:"100vh", background:"#0A1220", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:16}}>
      <div style={{
        width:40, height:40, borderRadius:"50%",
        border:"3px solid #2C3E5F", borderTop:"3px solid #FFD700",
        animation:"spin 0.8s linear infinite",
      }}/>
      <div style={{color:"#8494B0", fontSize:13}}>データを読み込み中...</div>
  ...
  return (
    <div style={{minHeight:"100vh", background:"#0A1220", fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif", color:"#E8EAF0"}}>
      ...
      <style>{`
        ...
        @media (max-width:640px){
          ...
          .bottomNav{
            ...
            background:#101D33; border-top:1px solid #2C3E5F;
      ...
      {/* ── ヘッダー ── */}
      <div style={{
        background:"linear-gradient(135deg,#16263F 0%,#101D33 100%)",
        borderBottom:"1px solid #2C3E5F", padding:"14px 18px",
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", maxWidth:980, margin:"0 auto"}}>
          <div style={{display:"flex", alignItems:"center", gap:10}}>
            ...
            <div>
              <div style={{fontWeight:700, fontSize:15, color:"#fff", letterSpacing:"0.05em"}}>ToDoリスト</div>
              <div style={{fontSize:10, color:"#8494B0", letterSpacing:"0.1em"}}>SALES TODO &amp; CALENDAR</div>
            </div>
          </div>
          <div className="topTabs" style={{display:"flex", gap:5, flexWrap:"wrap", justifyContent:"flex-end"}}>
            {TABS.map(([t,l])=>(
              <button key={t} onClick={()=>setTab(t)} style={{
                ...
                background:tab===t?"linear-gradient(135deg,#FFD700,#FF8C00)":"#1C2C48",
                color:tab===t?"#0A1220":"#8494B0", transition:"all 0.2s", whiteSpace:"nowrap",
              }}>{l}</button>
            ))}
          </div>
        </div>
      ...
      <div className="mainPad" style={{maxWidth:980, margin:"0 auto", padding:"12px 10px 100px"}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:9, marginBottom:12}}>
          {[
            ...
            <div key={s.label} style={{
              background:"#16263F", borderRadius:12, padding:"11px 13px",
              border:"1px solid #2C3E5F", position:"relative", overflow:"hidden",
            }}>
          ...
          <div>
            ...
            <div style={{display:"flex", gap:7, marginBottom:14}}>
              <input value={quickTitle} onChange={e=>setQuickTitle(e.target.value)}
                ...
                style={{flex:1, background:"#16263F", border:"1px solid #2C3E5F", borderRadius:10, padding:"10px 14px", color:"#E8EAF0", fontSize:13, outline:"none"}}/>
              <button onClick={quickAdd} style={{
                background:"linear-gradient(135deg,#FFD700,#FF8C00)", border:"none", borderRadius:10,
                padding:"0 18px", cursor:"pointer", fontSize:13, fontWeight:800, color:"#0A1220",
              }}>追加</button>
            </div>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:10, marginBottom:14}}>
              ...
              {/* 報告書欄(Firestore同期) */}
              <div style={{background:"#16263F", borderRadius:14, border:"1px solid #2C3E5F", padding:"12px 14px"}}>
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8}}>
                  <div style={{fontSize:11, color:"#8494B0", fontWeight:700}}>📝 本日提出報告書 <span style={{color:"#34D399"}}>☁同期</span></div>
                  <input type="date" value={reportDate} onChange={e=>setReportDate(e.target.value)}
                    style={{background:"#101D33", border:"1px solid #2C3E5F", borderRadius:6, padding:"3px 8px", color:"#8494B0", fontSize:11, outline:"none"}}/>
                </div>
                {/* 入力 */}
                <div style={{display:"flex", gap:6, marginBottom:8}}>
                  <input value={reportInput} onChange={e=>setReportInput(e.target.value)}
                    ...
                    style={{flex:1, background:"#101D33", border:"1px solid #2C3E5F", borderRadius:8, padding:"7px 10px", color:"#E8EAF0", fontSize:12, outline:"none"}}/>
                  <button onClick={addReport} style={{
                    background:"linear-gradient(135deg,#FFD700,#FF8C00)", border:"none", borderRadius:8, padding:"7px 12px", cursor:"pointer", fontSize:12, fontWeight:700, color:"#0A1220",
                  }}>追加</button>
                </div>
                {/* リスト */}
                {reportItems.filter(r=>r.date===reportDate).length===0
                  ? <div style={{fontSize:11, color:"#435470", textAlign:"center", padding:"8px 0"}}>本日の報告書なし</div>
                  : <div style={{display:"flex", flexWrap:"wrap", gap:6}}>
                      {reportItems.filter(r=>r.date===reportDate).map(r=>(
                        <div key={r.id} style={{display:"flex", alignItems:"center", gap:4, background:"#101D33", border:"1px solid #2C3E5F", borderRadius:20, padding:"4px 10px"}}>
              ...
              {/* ポモドーロタイマー */}
              <div style={{background:"#16263F", borderRadius:14, border:`1px solid ${pomoMode==="work"?"#FF444430":"#34C75930"}`, padding:"12px 16px"}}>
                <div style={{display:"flex", alignItems:"center", gap:12}}>
                  ...
                  <div style={{display:"flex", gap:6, alignItems:"center"}}>
                    ...
                    <button onClick={pomoReset} style={{background:"#1C2C48", border:"1px solid #2C3E5F", borderRadius:8, padding:"6px 10px", cursor:"pointer", fontSize:13, color:"#8494B0"}}>↺</button>
                    <div style={{fontSize:10, color:"#8494B0", whiteSpace:"nowrap"}}>🍅 {pomoCount}セット</div>
                  </div>
                </div>
              </div>
            </div>
              ...
              return (
                <div style={{display:"flex", gap:8, marginBottom:10}}>
                  {[
                    ...
                    <div key={s.label} style={{flex:1, background:"#16263F", borderRadius:10, border:"1px solid #2C3E5F", padding:"10px 12px", textAlign:"center"}}>
                      <div style={{fontSize:22, fontWeight:800, color:s.color, lineHeight:1}}>{s.value}</div>
                      <div style={{fontSize:10, color:"#8494B0", marginTop:3}}>{s.label}</div>
                    </div>
                  ))}
                </div>
              );
            ...
            {boardData && (()=>{
              ...
              return (
                <div style={{background:"#16263F", borderRadius:14, border:"1px solid #2C3E5F", padding:"12px 14px", marginBottom:14}}>
                  <div style={{display:"flex", alignItems:"center", marginBottom:10}}>
                    <div style={{fontSize:11, color:"#8494B0", fontWeight:700}}>📊 今月進捗 ({mLabel})</div>
                  </div>
                    ...
                    {prods.map(prod=>{
                      ...
                      return (
                        <div key={prod.key} style={{display:"flex", alignItems:"center", gap:10}}>
                          ...
                          <div style={{flex:1}}>
                            <div style={{height:6, background:"#101D33", borderRadius:3, overflow:"hidden"}}>
                          ...
                          <div style={{fontSize:10, color:"#8494B0", minWidth:60, textAlign:"right"}}>累計{cumP}%</div>
              ...
              <div style={{display:"flex", gap:5}}>
                {[["all","すべて"],["undone","未"],["progress","着手中"],["done","済"]].map(([v,l])=>(
                  <button key={v} onClick={()=>setListFilter(v)} style={{
                    ...
                    background:listFilter===v?"#FFD700":"#16263F",
                    color:listFilter===v?"#0A1220":"#8494B0",
                  }}>{l}</button>
                ))}
              </div>
              <div style={{marginLeft:"auto", display:"flex", gap:5, alignItems:"center"}}>
                <span style={{fontSize:11, color:"#5D6D8C"}}>並び替え</span>
                {[["date","日付"],["priority","優先度"]].map(([v,l])=>(
                  <button key={v} onClick={()=>setListSort(v)} style={{
                    ...
                    background:listSort===v?"#2C3E5F":"#16263F",
                    color:listSort===v?"#E8EAF0":"#5D6D8C",
                  }}>{l}</button>
                ))}
              </div>
            </div>
            ...
            <input value={listSearch} onChange={e=>setListSearch(e.target.value)}
              ...
              style={{width:"100%", boxSizing:"border-box", background:"#16263F", border:"1px solid #2C3E5F", borderRadius:10, padding:"9px 13px", color:"#E8EAF0", fontSize:13, outline:"none", marginBottom:10}}/>
            ...
            <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10}}>
              <div style={{fontSize:12, color:"#5D6D8C"}}>{listTodos.length}件</div>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:9}}>
              {listTodos.length===0&&(
                <div style={{background:"#16263F", borderRadius:14, padding:"36px 20px", textAlign:"center", color:"#8494B0", border:"1px dashed #2C3E5F"}}>
        ...
        {tab==="calendar" && (
          <div>
            <div style={{display:"flex", gap:5, marginBottom:14}}>
              {[["month","月次"],["3day","3日"],["day","日次"]].map(([v,l])=>(
                <button key={v} onClick={()=>setCalView(v)} style={{
                  ...
                  background:calView===v?"linear-gradient(135deg,#FFD700,#FF8C00)":"#1C2C48",
                  color:calView===v?"#0A1220":"#8494B0", transition:"all 0.2s",
                }}>{l}</button>
              ))}
            </div>
            ...
            {/* 月次 */}
            {calView==="month"&&(
              <div style={{background:"#16263F", borderRadius:18, border:"1px solid #2C3E5F", overflow:"hidden"}}>
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", padding:"13px 17px", borderBottom:"1px solid #2C3E5F"}}>
                  <button onClick={()=>setCurrentDate(new Date(year,month-1,1))} style={{background:"#101D33", border:"1px solid #2C3E5F", borderRadius:8, width:30, height:30, cursor:"pointer", color:"#E8EAF0", fontSize:16}}>‹</button>
                  <div style={{fontWeight:700, fontSize:17}}>{year}年 {MONTHS_JP[month]}</div>
                  <button onClick={()=>setCurrentDate(new Date(year,month+1,1))} style={{background:"#101D33", border:"1px solid #2C3E5F", borderRadius:8, width:30, height:30, cursor:"pointer", color:"#E8EAF0", fontSize:16}}>›</button>
                </div>
                <div style={{display:"grid", gridTemplateColumns:"repeat(7,1fr)", textAlign:"center"}}>
                  {DAYS_JP.map((d,i)=>(
                    <div key={d} style={{padding:"7px 0", fontSize:11, fontWeight:700, color:i===0?"#FF5252":i===6?"#4FC3F7":"#8494B0", borderBottom:"1px solid #2C3E5F"}}>{d}</div>
                  ))}
                </div>
                <div style={{display:"grid", gridTemplateColumns:"repeat(7,1fr)"}}>
                  {calendarDays.map((d,i)=>{
                    if(!d) return <div key={`e${i}`} style={{minHeight:70, borderRight:"1px solid #1C2C48", borderBottom:"1px solid #1C2C48"}}/>;
                    ...
                    return (
                      <div key={d} onClick={()=>{setSelectedDate(ds);setCalView("day");}} style={{
                        ...
                        borderRight:"1px solid #1C2C48", borderBottom:"1px solid #1C2C48",
                        background:isSel?"#223354":isToday?"#1A2A44":"transparent",
                      }}>
                        <div style={{
                          width:24, height:24, borderRadius:"50%",
                          display:"flex", alignItems:"center", justifyContent:"center",
                          background:isToday?"linear-gradient(135deg,#FFD700,#FF8C00)":"transparent",
                          color:isToday?"#0A1220":dow===0?"#FF5252":dow===6?"#4FC3F7":"#E8EAF0",
                        ...
                        {dt.length>2&&<div style={{fontSize:9,color:"#8494B0"}}>+{dt.length-2}</div>}
                      </div>
                    );
                  })}
                </div>
            ...
            {calView==="3day"&&(
              <div>
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12}}>
                  <button onClick={()=>setSelectedDate(addDays(selectedDate,-3))} style={{background:"#16263F", border:"1px solid #2C3E5F", borderRadius:8, padding:"6px 13px", cursor:"pointer", color:"#E8EAF0", fontSize:13}}>‹ 前3日</button>
                  <div style={{fontWeight:700, fontSize:13, color:"#8494B0"}}>{fmtDate(threeDays[0])} 〜 {fmtDate(threeDays[2])}</div>
                  <button onClick={()=>setSelectedDate(addDays(selectedDate,3))} style={{background:"#16263F", border:"1px solid #2C3E5F", borderRadius:8, padding:"6px 13px", cursor:"pointer", color:"#E8EAF0", fontSize:13}}>次3日 ›</button>
                </div>
                <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10}}>
                  {threeDays.map(ds=>{
                    const dt=todosFor(ds), d=new Date(ds), dow=d.getUTCDay(), isToday=ds===todayStr;
                    return (
                      <div key={ds} style={{background:"#16263F", borderRadius:14, border:isToday?"1px solid #FFD70060":"1px solid #2C3E5F", overflow:"hidden"}}>
                        <div style={{padding:"10px 12px", borderBottom:"1px solid #2C3E5F", background:isToday?"#FFD70010":"transparent", display:"flex", alignItems:"center", gap:6}}>
                          <div style={{fontWeight:800, fontSize:20, color:dow===0?"#FF5252":dow===6?"#4FC3F7":"#E8EAF0"}}>{d.getUTCDate()}</div>
                          <div>
                            <div style={{fontSize:10, color:"#8494B0"}}>{d.getUTCMonth()+1}月 {DAYS_JP[dow]}曜</div>
                            {isToday&&<span style={{fontSize:9, background:"#FFD700", color:"#0A1220", borderRadius:3, padding:"1px 5px", fontWeight:700}}>TODAY</span>}
                          </div>
                          <div style={{marginLeft:"auto", background:"#101D33", borderRadius:10, padding:"2px 8px", fontSize:11, color:"#8494B0"}}>{dt.length}件</div>
                        </div>
                        <div style={{padding:"8px", display:"flex", flexDirection:"column", gap:6}}>
                          {dt.length===0&&<div style={{textAlign:"center", color:"#435470", fontSize:11, padding:"10px 0"}}>タスクなし</div>}
                          {dt.map(t=>{
                            const done=isDoneTodo(t);
                            const pri=PRIORITIES[t.priority]||PRIORITIES.medium;
                            return (
                              <div key={t.id} onClick={()=>{setSelectedDate(ds);setCalView("day");}} style={{background:"#101D33", borderRadius:8, padding:"8px 10px", border:`1px solid ${done?"#2C3E5F":pri.color+"40"}`, opacity:done?0.5:1, cursor:"pointer"}}>
                          ...
                          <button onClick={()=>{setSelectedDate(ds);openAdd(ds);}} style={{width:"100%", background:"transparent", border:"1px dashed #2C3E5F", borderRadius:8, padding:"6px", cursor:"pointer", color:"#5D6D8C", fontSize:11}}>+ 追加</button>
                        </div>
                      </div>
                    );
                  })}
            ...
            {calView==="day"&&(
              <div>
                <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:13}}>
                  <button onClick={()=>setSelectedDate(addDays(selectedDate,-1))} style={{background:"#16263F", border:"1px solid #2C3E5F", borderRadius:8, padding:"6px 13px", cursor:"pointer", color:"#E8EAF0", fontSize:13}}>‹ 前日</button>
                  <div style={{textAlign:"center"}}>
                    <div style={{fontWeight:800, fontSize:20}}>{fmtDate(selectedDate)}</div>
                    <div style={{fontSize:12, color:"#8494B0"}}>
                      {DAYS_JP[new Date(selectedDate).getUTCDay()]}曜日
                      {selectedDate===todayStr&&<span style={{marginLeft:6, background:"#FFD700", color:"#0A1220", borderRadius:4, padding:"1px 6px", fontSize:10, fontWeight:700}}>TODAY</span>}
                    </div>
                  </div>
                  <button onClick={()=>setSelectedDate(addDays(selectedDate,1))} style={{background:"#16263F", border:"1px solid #2C3E5F", borderRadius:8, padding:"6px 13px", cursor:"pointer", color:"#E8EAF0", fontSize:13}}>翌日 ›</button>
                </div>
      ...
      {showDealModal&&(
        <div style={{position:"fixed",inset:0,background:"#000000BB",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:16}}
          ...
          <div style={{background:"#16263F",borderRadius:20,padding:22,width:"100%",maxWidth:440,border:"1px solid #2C3E5F",maxHeight:"92vh",overflowY:"auto"}}>
            <div style={{fontWeight:800,fontSize:17,marginBottom:18}}>{editDeal?"案件編集":"案件追加"}</div>
            <div style={{display:"flex",flexDirection:"column",gap:13}}>
              {/* 会社名 */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>会社名</label>
                <input value={dealForm.company} onChange={e=>setDealForm({...dealForm,company:e.target.value})}
                  placeholder="例：山田商事" style={inputStyle}/>
              </div>
              {/* ステージ */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>ステージ</label>
                <div style={{display:"flex",gap:6}}>
                  {STAGES.map(s=>(
                    <button key={s.key} onClick={()=>setDealForm({...dealForm,stage:s.key})} style={{
                      ...
                      background:dealForm.stage===s.key?s.color+"30":"#101D33",
                      color:dealForm.stage===s.key?s.color:"#5D6D8C",
              ...
              {/* 商品 */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>商品（複数選択可）</label>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {PRODUCTS.map(p=>{
                    ...
                    return (
                      <button key={p.key} onClick={()=>{
                        ...
                        background:active?p.color+"18":"#101D33",color:active?p.color:"#5D6D8C",
              ...
              {/* 金額 */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>金額（円）</label>
              ...
              {/* 担当者 */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>担当者名</label>
                <input value={dealForm.person} onChange={e=>setDealForm({...dealForm,person:e.target.value})}
                  placeholder="例：田中部長" style={inputStyle}/>
              </div>
              {/* 次回アクション日 */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>次回アクション日 <span style={{color:"#5D6D8C",fontWeight:400}}>(設定するとフォロータスクを自動作成)</span></label>
                <input type="date" value={dealForm.nextAction} onChange={e=>setDealForm({...dealForm,nextAction:e.target.value})} style={inputStyle}/>
              </div>
              {/* 備考 */}
              <div>
                <label style={{fontSize:11,color:"#8494B0",display:"block",marginBottom:5}}>備考</label>
              ...
              <div style={{display:"flex",gap:10,marginTop:2}}>
                <button onClick={()=>setShowDealModal(false)} style={{flex:1,padding:"11px",borderRadius:12,border:"1px solid #2C3E5F",background:"transparent",color:"#8494B0",cursor:"pointer",fontSize:13,fontWeight:600}}>キャンセル</button>
                <button onClick={saveDeal} style={{flex:2,padding:"11px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#FFD700,#FF8C00)",color:"#0A1220",cursor:"pointer",fontSize:13,fontWeight:800}}>保存する</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <button className="fab" onClick={()=>openAdd()} style={{
        ...
        background:"linear-gradient(135deg,#FFD700,#FF8C00)",
        border:"none", cursor:"pointer", fontSize:26, color:"#0A1220",
        ...
        <div style={{position:"fixed", inset:0, background:"#000000BB", display:"flex", alignItems:"center", justifyContent:"center", zIndex:300, padding:16}}
          ...
          <div style={{background:"#16263F", borderRadius:20, padding:22, width:"100%", maxWidth:440, border:"1px solid #2C3E5F", maxHeight:"92vh", overflowY:"auto"}}>
            ...
            <div style={{display:"flex", flexDirection:"column", gap:13}}>
              ...
              {/* テンプレート */}
              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>テンプレート <span style={{color:"#5D6D8C",fontWeight:400}}>(タップで反映)</span></label>
                <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
                  {templates.map((tp,i)=>(
                    <span key={i} style={{display:"inline-flex", alignItems:"center", gap:4, background:"#101D33", border:"1px solid #2C3E5F", borderRadius:16, padding:"4px 4px 4px 10px"}}>
                      ...
                      <button onClick={()=>deleteTemplate(i)} style={{background:"none", border:"none", cursor:"pointer", color:"#5D6D8C", fontSize:10, padding:"0 4px"}}>×</button>
                    </span>
                  ))}
                  <button onClick={saveAsTemplate} style={{background:"#1C2C48", border:"1px dashed #2C3E5F", borderRadius:16, padding:"4px 10px", cursor:"pointer", color:"#8494B0", fontSize:11}}>+ 現在の内容を登録</button>
                </div>
              </div>

              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>タスク名</label>
                <input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="例：山田商事 提案書作成" style={inputStyle}/>
              </div>
              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>日付</label>
                <input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} style={inputStyle}/>
              </div>
              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>繰り返し <span style={{color:"#5D6D8C",fontWeight:400}}>(済にすると次回分を自動作成)</span></label>
                <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
                  {Object.entries(REPEATS).map(([k,v])=>(
                    <button key={k} onClick={()=>setForm({...form,repeat:k})} style={{
                      ...
                      background:(form.repeat||"")===k?"#4FC3F730":"#101D33",
                      color:(form.repeat||"")===k?"#4FC3F7":"#5D6D8C",
              ...
              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>優先度</label>
                <div style={{display:"flex", gap:8}}>
                  {Object.entries(PRIORITIES).map(([k,v])=>(
                    <button key={k} onClick={()=>setForm({...form,priority:k})} style={{
                      ...
                      background:form.priority===k?v.bg:"#101D33", color:form.priority===k?v.color:"#5D6D8C",
              ...
              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>ステータス</label>
                <div style={{display:"flex", gap:8}}>
                  {Object.entries(STATUSES).map(([k,v])=>(
                    <button key={k} onClick={()=>setForm({...form,status:k})} style={{
                      flex:1, padding:"8px", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:700,
                      background:form.status===k?v.bg:"#101D33", color:form.status===k?v.color:"#5D6D8C",
              ...
              <CategoryPicker/>
              <div>
                <label style={{fontSize:11, color:"#8494B0", display:"block", marginBottom:5}}>メモ</label>
              ...
              <div style={{display:"flex", gap:10, marginTop:2}}>
                <button onClick={()=>setShowModal(false)} style={{flex:1, padding:"11px", borderRadius:12, border:"1px solid #2C3E5F", background:"transparent", color:"#8494B0", cursor:"pointer", fontSize:13, fontWeight:600}}>キャンセル</button>
                <button onClick={saveTodo} disabled={saving} style={{flex:2, padding:"11px", borderRadius:12, border:"none", background:saving?"#2C3E5F":"linear-gradient(135deg,#FFD700,#FF8C00)", color:saving?"#8494B0":"#0A1220", cursor:saving?"not-allowed":"pointer", fontSize:13, fontWeight:800}}>
                  {saving?"保存中...":"保存する"}
                </button>
              </div>
            </div>
